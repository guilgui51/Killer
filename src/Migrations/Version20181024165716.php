 ...
 */
final class Version20181024165716 extends AbstractMigration
{
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__jeudesociete AS SELECT id, name, price, min_player, max_player, shop_link, owned, interest, available FROM jeudesociete');
        $this->addSql('DROP TABLE jeudesociete');
        $this->addSql('CREATE TABLE jeudesociete (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(255) NOT NULL COLLATE BINARY, available BOOLEAN DEFAULT NULL, price DOUBLE PRECISION DEFAULT NULL, min_player INTEGER DEFAULT NULL, max_player INTEGER DEFAULT NULL, shop_link VARCHAR(800) DEFAULT NULL, owned BOOLEAN DEFAULT NULL, interest INTEGER DEFAULT NULL, picture VARCHAR(255) DEFAULT NULL)');
        $this->addSql('INSERT INTO jeudesociete (id, name, price, min_player, max_player, shop_link, owned, interest, available) SELECT id, name, price, min_player, max_player, shop_link, owned, interest, available FROM __temp__jeudesociete');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__jeudesociete AS SELECT id, name, price, min_player, max_player, shop_link, owned, interest, available FROM jeudesociete');
        $this->addSql('DROP TABLE jeudesociete');
        $this->addSql('CREATE TABLE jeudesociete (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(255) NOT NULL, available BOOLEAN DEFAULT NULL, price DOUBLE PRECISION NOT NULL, min_player INTEGER NOT NULL, max_player INTEGER NOT NULL, shop_link VARCHAR(800) NOT NULL COLLATE BINARY, owned BOOLEAN NOT NULL, interest INTEGER NOT NULL)');
        $this->addSql('INSERT INTO jeudesociete (id, name, price, min_player, max_player, shop_link, owned, interest, available) SELECT id, name, price, min_player, max_player, shop_link, owned, interest, available FROM __temp__jeudesociete');
        $this->addSql('DROP TABLE __temp__jeudesociete');
        $this->addSql('DROP INDEX UNIQ_29935C985DFCD4B8');
        $this->addSql('CREATE TEMPORARY TABLE __temp__killer_party AS SELECT id, winner_id, starting_date, name, uid, is_over FROM killer_party');
